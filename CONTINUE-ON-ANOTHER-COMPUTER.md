...
## 현재 상태 (2026-08-05 기준)
- 노트 타입: 노트(마크다운) · 🎨 캔버스 · 📊 데이터(테이블). + 🗄 데이터베이스는 전역 뷰(사이드바 검색 위 버튼).
- 뷰 전환은 전부 `showView(name)` 한 곳을 거침. 좌측 사이드바 접기는 제거됨(우측 패널만 토글).
- 노트 편집기: "쓰기모드"(편집|미리보기 분할, 드래그 구분선·터치 지원) / "읽기모드"(미리보기). 읽기모드엔 `⧉ 비교` 버튼으로 2카드 나란히 보기(데스크탑/태블릿 전용).
- 콜아웃은 💡 버튼 → 4종 팝오버 선택. 캔버스: Undo/Redo·다중선택·스냅·섹션·카드노드·커넥터 컨텍스트 툴바, 도형 Enter로 텍스트 편집, 터치 선연결(선택 시 앵커 표시), 마름모 정사각 유지. 저장은 JSON.
- 스키마: `notes`에 `type`/`content_len`/`properties`/`deleted_at` 컬럼 + user_id 복합 인덱스, `checklist_items` 테이블(할 일 동기화). 본문은 지연 로딩(`ensureContentLoaded`). 상세는 `CHANGELOG.md` 참고.
- 특수 카드(캔버스/데이터) 데이터는 여전히 `notes.content`에 JSON으로 저장.

> **오늘(2026-08-05) 웹에 반영한 편집기/모바일/캔버스 개선은 안드로이드 앱엔 아직 없음.** 실기기에 넣으려면 이 컴퓨터에서 `npx cap sync android` 후 재빌드 필요(특히 Android 뒤로가기·사이드바 탭 닫기·캔버스 터치 선연결).
